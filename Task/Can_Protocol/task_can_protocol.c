...
static uint16_t can_protocol_task_assignctrl(uint8_t Ch, uint8_t Value);
                        ...
                        case CAN_PROTOCOL_CMD_ASSIGN_CTRL:
                            {
                                ...
                                res = can_protocol_task_assignctrl(u8Channal, 1);
                                //未选中
                                if (res)
                                {
                                    gTsCanFrame.data[CAN_PROTOCOL_RESP_DATA_ROLL_COUNTER] = gTsCanFramePtr->data[CAN_PROTOCOL_REQ_DATA_ROLL_COUNTER];
...
#if 1
static uint16_t
can_protocol_task_assignctrl(uint8_t Ch, uint8_t Value)
{
    uint16_t res = 1;
    switch (Ch)
    {
